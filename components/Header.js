...
function Header() {
  // added router reference for create-post
  ...
  return (
    ...
    border-[#FF3366]'>
        ...
        {/* STEP 2 : add createPost and signIn-Out buttons */}
        <div className='flex gap-4'>
            {/* 2.1 : createPost */}
            {/* 3 router/path added */}
            <button onClick={()=>router.push('/create-post')} className='bg-black p-1 px-3 text-white rounded-full text-[12px]'>
              <span className='hidden sm:block'> CREATE POST</span> 
              <HiOutlinePencilSquare className='sm:hidden text-[17px]' />
            </button>
            {/* 2.2 : signIn/signOut */}
            {/* login/logout btn condition : if session not active already ... */}
            {!session? 
            ...
            {/* 2.3 : profile pic */}
            {/* if session active : show pp */}
            {session?  
                <Image src={session? session?.user?.image : USER_IMAGE} alt='user image'  className='rounded-full cursor-pointer' onClick={()=>router.push('/profile')} width={40} height={40} />
              :
                null
            }
